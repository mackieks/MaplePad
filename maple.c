...
int main() {
	stdio_init_all();
	// set_sys_clock_khz(250000, true); // Overclock seems to lead to instability over time
	adc_init();
	adc_set_clkdiv(0);
	...
	// 	// if( (ControllerPacket.Controller.JoyX > 0x80) && (ControllerPacket.Controller.JoyY < 0x80) )
	// 	// 	drawEllipse(map(ControllerPacket.Controller.JoyX, 0x00, 0xff, 69, 27), map(ControllerPacket.Controller.JoyY, 0x00, 0xff, 53, 10), map(r, 0x00, 0x80, 13, 4), 13, -theta);
	// 	// else if( (ControllerPacket.Controller.JoyX < 0x80) && (ControllerPacket.Controller.JoyY > 0x80) )
	// 	// 	drawEllipse(map(ControllerPacket.Controller.JoyX, 0x00, 0xff, 69, 27), map(ControllerPacket.Controller.JoyY, 0x00, 0xff, 53, 10), map(r, 0x00, 0x80, 13, 4), 13, -theta);
	// 	// else
	// 	// 	drawEllipse(map(ControllerPacket.Controller.JoyX, 0x00, 0xff, 69, 27), map(ControllerPacket.Controller.JoyY, 0x00, 0xff, 53, 10), map(r, 0x00, 0x80, 13, 4), 13, theta);
	// 	fillCircle(48, 32, 5, 0xffff);
	// 	//sleep_ms();
	// 	updateSSD1331();
	...
	// while(gpio_get(ButtonInfos[4].InputIO) && gpio_get(ButtonInfos[5].InputIO)){};
